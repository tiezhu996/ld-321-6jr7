...
已派单任务由调度看板点击「完工登记」，提交实际工时、油耗（升）、作业面积（亩）：

1. 校验通过后生成与任务一一对应的**可追溯作业记录**（`work_records.task_id` 唯一），任务置为「已完成」并记录完工时间；农机恢复「空闲」、驾驶员恢复「可派单」。
2. 每次完工按实际工时扣减该农机的**保养剩余工时**（同步扣减保养提醒剩余工时），扣至零时农机自动转为「维修中」、提醒升级为紧急，并**阻止继续派单**。
3. **重复或并发完工只允许成功一次**：事务内行锁串行化 + 唯一索引兜底，第二次起返回 `409`，不改变记录、任务、农机或驾驶员状态。
4. 字段缺失（任一必填项）或任务状态不允许（非「已派单」）返回 `400/409`，全程不产生任何状态变更。
5. 完工后看板缓存自动失效，刷新即可看到任务列表、作业统计、保养提醒、农机与驾驶员状态同步更新。
...
| POST | /tasks/:id/complete | 完工登记（实际工时/油耗/作业面积），生成可追溯作业记录 | - |
